...
const YeniBlogPage = () => {
  ...
  // Odak anahtar kelime - sadece analiz için (DB'ye kaydetmiyoruz)
  ...
  const highlightSuggestions = (quill: any, items: LinkSuggestion[]) => {
    ...
    items.forEach(item => {
      item.spans.forEach(({ start, length }) => {
        try {
          // Zaten linkli ise atla
          const fmt = quill.getFormat(start, length);
          if ((fmt as any)?.link) return;
          quill.formatText(start, length, { background: '#FEF08A' }); // yellow-200
        } catch {}
      });
    });

    // Vurguya tıklamada popover gösterimi: selection-change ile yakala
  ...
  const scanLinkSuggestions = async () => {
    ...
    try {
      ...
      // Diğer yazıları getir (başlık + slug)
      ...
      (posts || [])
        ...
        .forEach((p) => {
          ...
          phrases.forEach((ph) => {
            ...
            // Kelime sınırlarını koruyan, lookbehind kullanmadan çalışan desen
  ...
  // Link kopyalama: popover'dan slug bazlı
